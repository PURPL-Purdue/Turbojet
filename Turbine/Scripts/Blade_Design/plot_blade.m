function plot_blade(blade, betas, R, R_LE, R_TE)
    figure(1)
    hold on
    xlim([-0.25,1.5])
    ylim([-0.25,1.5])
    ...
    % Points 1-5
    scatter(blade.points_x, blade.points_y,5, "filled")
    ...
    plot_arcs(blade.points_x, blade.points_y, betas, R, R_LE, R_TE)
end
...
function plot_arcs(x, y, betas, R, R_LE, R_TE)
...
function arc(x,y,r, x1, x2, y1, y2)
    ...
    begin = atan2(y1-y,x1-x);
    stop = atan2(y2-y,x2-x);
    if begin > stop
        stop = stop + 2 * pi;
    end

    theta = begin:pi/100:stop;
    xunit = r * cos(theta) + x;
    yunit = r * sin(theta) + y;
